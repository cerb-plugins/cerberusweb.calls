...
<table cellpadding="2" cellspacing="0" border="0" width="100%" id="{$view->id}_actions">
	...
	<tr>
		<td colspan="2" valign="top">
			...
			{if $active_worker->hasPriv('calls.actions.update_all')}<button id="btn{$view->id}BulkUpdate" type="button" onclick="genericAjaxPopup('peek','c=calls&a=showCallsBulkPanel&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),null,false,'500');"><span class="cerb-sprite2 sprite-folder-gear"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
		</td>
	</tr>
	{/if}
	<tr>
