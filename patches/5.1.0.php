...
if(isset($columns['name']))
	$db->ExecuteMaster("ALTER TABLE call_entry DROP COLUMN name");

if(isset($columns['org_id']))
	$db->ExecuteMaster("ALTER TABLE call_entry DROP COLUMN org_id");

if(isset($columns['worker_id']))
	$db->ExecuteMaster("ALTER TABLE call_entry DROP COLUMN worker_id");

if(!isset($columns['subject']))
	$db->ExecuteMaster("ALTER TABLE call_entry ADD COLUMN subject VARCHAR(255) NOT NULL DEFAULT ''");
	
if(!isset($columns['is_outgoing']))
	$db->ExecuteMaster("ALTER TABLE call_entry ADD COLUMN is_outgoing TINYINT(1) UNSIGNED NOT NULL DEFAULT 0");
...
if(!isset($indexes['is_closed']))
    $db->ExecuteMaster('ALTER TABLE call_entry ADD INDEX is_closed (is_closed)');
